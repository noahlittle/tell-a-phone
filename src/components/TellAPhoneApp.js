...
const TellAPhoneApp = () => {
  ...
  const appendAudioChunk = useCallback((audioChunk) => {
    if (!audioContextRef.current || !isAudioEnabled) {
      return;
    }

    const arrayBuffer = audioChunk.buffer;
  ...
  const toggleQueue = useCallback(async () => {
    if (socketRef.current) {
      if (isInQueue) {
        ...
        try {
          await handleUserInteraction(); // Ensure AudioContext is resumed
  ...
  }, [isInQueue, handleUserInteraction]);
  ...
  }, [appendAudioChunk, startBroadcasting, stopBroadcasting, isBroadcasting]);
...
if (showUsernameDialog) {
  return (
    <Dialog open={showUsernameDialog}>
      <DialogContent className="sm:max-w-[425px] bg-gray-800 text-gray-100" aria-describedby="dialog-description">
        <DialogHeader>
        ...
        <p id="dialog-description" className="sr-only">This dialog allows you to set your username and join the audio broadcast.</p>
      </DialogContent>
    </Dialog>
  );
}
